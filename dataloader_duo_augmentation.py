...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode, logger):
        ...
        # X_train is now a 3-D tensor where each row is a 30 second recording, i.e. X_train[0, :, :] returns the first 30 second recording.
         
        y_train = dataset["labels"]
        # y_train is a 1-D vector with #of rows total entries. It contains a label for each row in the X_train. 
        ...
        if training_mode == "self_supervised":  # no need to apply Augmentations in other modes
            self.aug1, self.aug2 = DataTransform(self.x_data, config, logger) # See augmentations.py
    ...
    def __getitem__(self, index):
        if self.training_mode == "self_supervised":
            ...
            return self.x_data[index], self.y_data[index], self.y_data[index], self.y_data[index] # Must return additional values not to throw errors in trainer.model_train
