...
def DataTransform(training_data, config, logger):
    ...
    aug1, aug2 = determine_augmentation_set(config, logger)
    # >> Returns either one or two augmentations and its hyperparameters

    def augmenter():
        # Initialize empty array to hold the augmented samples
        n = training_data.shape[0]
        augmented_data = torch.empty(n, 1, 3000)

        # Augment the training_data using only aug1(.) OR by composing aug1(aug2(.))
        if aug2[0] == None:
            for i in range(n):
                aug = aug1[0](training_data[i], aug1[1]) # aug1[0], aug1[1] >> first augmentation, its hyperparameters
        ...
        # >> Returns a (n, 1, 3000) tensor with augmented data

    augmented_data_1 = augmenter()
    augmented_data_2 = augmenter()
