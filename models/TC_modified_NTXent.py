...
class TC(nn.Module):
    def __init__(self, configs, device):
        ...
        # Take as input a flattened 128*127-d vector, output a 128-d vector
        # 512-d hidden from https://arxiv.org/pdf/2212.11491.pdf
        self.projection_head = nn.Sequential(
            nn.Linear(configs.final_out_channels*configs.features_len, 512),
            nn.ReLU(inplace=True),
            nn.Linear(512, 128),
        )
    ...
    def forward(self, features_source, features_aug):
        ...
        c_aug = self.seq_transformer(forward_seq)
        ...
        # Flatten encoder embeddings from (128, 127, 128) >> (128, 16256)
        z_source = z_source.flatten(start_dim=1, end_dim=2)
        z_aug = z_aug.flatten(start_dim=1, end_dim=2)
        # Pass through non-linear projection head
        projected_source = self.projection_head(z_source) 
        projected_aug = self.projection_head(z_aug)
        # >> (128, 128)

        # Compute to NCE loss
        nce = 0
        for i in np.arange(1): # 0, self.timestep
        ...
        return nce, projected_source, projected_aug
