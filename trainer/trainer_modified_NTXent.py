...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    ...
    if training_mode == "self_supervised":
        ...
        nt_xent_losses = torch.empty(size=(config.num_epoch, len(train_dl))).to(device)
        # Stores the loss of every batch in eaach epoch


    # Loop through each epoch, training and evaluating the model
    for epoch in range(1, config.num_epoch + 1): 
        # Train
        train_loss, train_acc, nce_loss, nt_xent_loss  = model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_dl, config, device, training_mode, logger, experiment_log_dir)
        ...
        if training_mode == "self_supervised":
            ...
            # Don't evaluate, only report training loss
            logger.debug(f'\nEpoch : {epoch}\n'
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode, logger, experiment_log_dir):
    total_loss = []
    total_acc = []
    model.train()
    ...
    for batch_idx, (source, target, aug) in enumerate(train_loader):
        ...
        if training_mode == "self_supervised":
            ...
            # Call TC model, obtain NCE loss and the projected encoder embeddings
            tc_loss, projected_source, projected_aug = temporal_contr_model(features_source, features_aug)
            ...
            nt_xent_loss = nt_xent_criterion(projected_source, projected_aug)
    ...
    return total_loss, total_acc, nce_losses, nt_xent_losses
