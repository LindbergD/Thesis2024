...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    ...
    for batch_idx, (source, target, aug1, aug2) in enumerate(train_loader):
        ...
        if training_mode == "self_supervised":
            ...
            predictions_1, features_1 = model(aug1)
            predictions_2, features_2 = model(aug2)
            ...
            tc_loss_1, projected_features_1 = temporal_contr_model(features_1, features_2)
            tc_loss_2, projected_features_2 = temporal_contr_model(features_2, features_1)
                                    
            # normalize projection feature vectors
            zis = projected_features_1
            zjs = projected_features_2
...
def model_evaluate(model, temporal_contr_model, test_dl, device, training_mode):
    ...
    with torch.no_grad():
        for source, target, _, _ in test_dl:
            source, target = source.float().to(device), target.long().to(device)
