...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    ...
    for epoch in range(1, config.num_epoch + 1): 
        ...
        # train_dl is a DataLoader object, but we can access the cusotm Load_Dataset object and change the augmentations
        # Create new augmentations of the data for this epoch
        if training_mode == "self_supervised":
            train_dl.dataset.aug = DataTransform(train_dl.dataset.x_data, config)

        train_loss, train_acc, nt_xent_loss, nce_loss = model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_dl, config, device, training_mode, logger, experiment_log_dir)
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode, logger, experiment_log_dir):
    ...
    for batch_idx, (source, target, aug) in enumerate(train_loader):
        ...
        if training_mode == "self_supervised":
            ...
            # run the TC model on embeddings, call twice to make predictions both ways
