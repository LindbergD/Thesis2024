...
def _calc_metrics(pred_labels, true_labels, log_dir, home_path):
    ...
    r = classification_report(y_true=true_labels, y_pred=pred_labels, digits=6, output_dict=True)
...
def _logger(logger_name, level=logging.DEBUG):
    ...
    format_string = ("%(asctime)s — %(filename)s — %(levelname)s — %(funcName)s:"
                     "%(lineno)d — %(message)s")
...
def copy_Files(destination, data_type, config_version):
    ...
    copy(f"config_files/{config_version}_Configs.py", os.path.join(destination_dir, f"{config_version}_Configs.py"))
